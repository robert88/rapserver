...
function matchValue(parentStr,templaterParse,compilerParse,idx,len,count,templateIndex){
    ...
    if(!compilerParse|| hasTemplateTag(compilerParse)){
        ...
        let isNotNextMatchTempalte = true;
        ...
        if(templaterParse){
            ...
            isNotNextMatchTempalte =  !nextCompilerParse || !nextTemplateParse || (nextTemplateParse.match!=nextCompilerParse.match);
        }

        //存在往下匹配
        if (isNotNextMatchTempalte) {
            count.count++;
